...
//------------------------------------------------------------>	public static SLAContract customerSLAContract = SLAContract.getInstance(PowerAndPerformance.CUSTOMER_SLA_CONTRACT);
	 ...
	 */
	/*
	 * List<Host> HostList; List<Vm> VmList; Map<Long, Long> OlddynamicVmHostMap;
	 * List<Long> TargetHostList;
	 */
	/*
	 * public DynamicChromosomeDAAS(List<Host> hostList, List<Vm> vmList)
	 * {//--------------------------------------------------------> genes = new
	 * HashMap<Long, Long>(GeneticAlgorithmDriverWA.dynamicVmHostMap.size());
	 * this.HostList = hostList; this.VmList = vmList; }
	 */
	...
	public DynamicChromosomeWA initialize(int i, List<Host> hostList, List<Vm> vmList) throws IllegalArgumentException {
		...
		if(i == 0){
			GADriverWA.dynamicVmHostMap.forEach((vm, server) -> {
				if(!LocalControllerWA.VmstoMigrateFromOverloadedHosts.isEmpty()){
					fillgenesForOverloadedHostVms(vm, server);
				}else {
					genes.put(vm, server);
				}
			});	
			genes.forEach((vm, server)->{
				if(server == null) {
		...
		if (i >= 1) {
			GADriverWA.dynamicVmHostMap.forEach((vm, server) -> {
				...
				if (LocalControllerWA.VmstoMigrateFromOverloadedAndUnderloadedHosts.stream()
						.anyMatch(Vm -> (Vm.getId() == vm))) {
					List<Vm> ListVm = LocalControllerWA.VmstoMigrateFromOverloadedAndUnderloadedHosts.stream()
	...
	private void fillgenesForOverloadedHostVms(Long vm, Long server) {
		if(LocalControllerWA.VmstoMigrateFromOverloadedHosts.stream()
				.anyMatch(Vm -> (Vm.getId() == vm))) {
	...
	public void serverCurrentUtilization(Map<Long, ArrayList<Long>> NewServerwithVmlist) {
		...
		NewServerwithVmlist.forEach((server, vmList) -> {
			...
			vmList.forEach(vm -> {			
				...
				vmsRAMUtil += vm_List.get(vm.intValue()).getRam().getAllocatedResource();//getCurrentRequestedRam() - does not give the accurate current requested ram for WA
			});
...
//		System.out.println(normalizedDcPower);
	...
	public double slaViolations() {
		...
		ServerwithVmlist.forEach((Server, Vmlist) -> {
			...
			if ((serverRamCapacity < AllvmsRAMCapacityRequirement)
				...
				for(Long Vm : Vmlist) {
					if(LocalControllerWA.VmstoMigrateFromOverloadedAndUnderloadedHosts.contains(vm_List.get(Vm.intValue()))) {
	...
	public int getSLAVUP() {
		ServerwithVmlist.forEach((server, vmList) -> {
			...
			double UpperUtilizationThreshold = GlobalController.HostUpperUtilizationThresholdWA;
			if(vmisPresent == true) {
