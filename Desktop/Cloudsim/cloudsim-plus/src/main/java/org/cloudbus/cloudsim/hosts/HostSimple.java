...
public class HostSimple implements Host {
    ...
    private String description;//Added newly-------------------------------------------------------------------------------------------------------------------------- 
...
//        if(!allocateResourcesForVm(vm, true)){
  //          vmsMigratingIn.remove(vm);
    ...
    public long getRamUtilization() {
    	 /** 
         *  Previously ram utilization was based on the the allocated resources.it does not consider the utilization model of vms. 
         *  From now on ram utilization of a host will be decided from its vm utilization model.
         */
        return ramProvisioner.getTotalAllocatedResource();
        	
		/*
		 * double UsageAsPerUtilizationModel=0.0;
		 * 
		 * for (Vm vm : vmList) { UsageAsPerUtilizationModel +=
		 * vm.getCloudletScheduler().getCurrentRequestedRamPercentUtilization(); }
		 * UsageAsPerUtilizationModel = UsageAsPerUtilizationModel/vmList.size(); double
		 * CurrentRamUtilization = ((double) ramProvisioner.getTotalAllocatedResource())
		 * * UsageAsPerUtilizationModel;
		 * 
		 * return (long) CurrentRamUtilization;
		 */
    }
    ...
    //----------------> vmCreatedList contains duplicate Vm's. (i.e same vm appears multiple times) as the vm gets migrated and again comes back 
//    	to its previous Host. Hence it gets created again. Because of this same vm's utilization gets counted multiple times ( n times it gets created).
    ...
    @Override
    public Host setDescription(final String description) {
