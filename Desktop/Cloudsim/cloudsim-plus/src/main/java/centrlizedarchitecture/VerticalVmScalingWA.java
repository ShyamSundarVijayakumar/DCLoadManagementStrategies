...
public class VerticalVmScalingWA extends VerticalVmScalingSimple{
	 ...
	 public VerticalVmScalingWA(final Class<? extends ResourceManageable> resourceClassToScale, final double scalingFactor){
	    ...
	    public boolean isVmUnderloaded() {
	    	...
	    	Map<Long, Long> VmServerMap = centrlizedarchitecture.centralManager.bestDynamicVmServerMapWA;
	    	...
	    	if((CurrentTime < 5*300) || (getResource().getCapacity() < 250) || (inMigration)) {
	    		return false;
	    	}else {
	    		...
	    		if((lastThreeutilizations[0] < lowerUtilizationThreshold) && (lastThreeutilizations[1] < lowerUtilizationThreshold)
	    				&& (lastThreeutilizations[2] < lowerUtilizationThreshold) 
	    				&& (lastThreeutilizations[3] < lowerUtilizationThreshold) && (lastThreeutilizations[4] < lowerUtilizationThreshold)){
	    				return true;	
	    		} else {
	    			return false;
	    		}
	    ...
	    public boolean isVmOverloaded() {
	    	if((CurrentTime < 5*300) || (!getVm().getHost().getVmsMigratingIn().isEmpty())) {
	    		return false;
	    	}else {
	    		for(int i = 0; i < 5 ; i++) {
	    			lastThreeutilizations[i] = ramUtilizationhistory.get((ramUtilizationhistory.size()-1) - i);	
	    		...
	    		if((lastThreeutilizations[0] > upperthreshold) && (lastThreeutilizations[1] > upperthreshold)
	    				&& (lastThreeutilizations[2] > upperthreshold) 
	    				&& (lastThreeutilizations[3] > upperthreshold)&& (lastThreeutilizations[4] < upperthreshold)){
	    				return true;	
	    		} else {
	    			return false;
	    		}
