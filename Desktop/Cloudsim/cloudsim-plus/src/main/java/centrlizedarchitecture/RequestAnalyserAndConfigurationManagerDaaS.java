...
//	private Map<Integer, Integer> VMTypeWithItsOfflineDelayMap = new HashMap<>();
	 ...
	 */	 
	 ...
	 private void listenerToInitiateNextDayVms( EventInfo info){
		 ...
		 if(!VmlistForDispatcher.isEmpty()) {
				/*
			 ...
			 if(!(VMIdWithItsOfflineDelayMapSorted.values().stream().findFirst().get() == time)) {
				 CM.updateVmListDaasAfterOffline(VmlistForDispatcher,time); 
				 VmlistForDispatcher = new ArrayList<>();
			 }
		 }
	 }
   ...
   //		   System.out.println("VMtype"+vmQueueDC3);//for debugging
	//		 System.out.println("VMid is going for creation"+ VMid);
   ...
   //		   VmWithItsOfflineDelayMap.remove(firstOflineTime,treeMap1.remove(VMType,Vmid));
	//	System.out.println("vmQueue after vms are given creation request"+ vmQueue);
			 for(Vm vm : vmQueue) {
				 if(vm.getId() == VMid) {
					 Vm ThisVm =vm;
					 Vmlist.add(ThisVm);
		...
		case 1:
			...
			Cloudlet cloudlet = new CloudletSimple(LetID++, length, pesNumber)
		 		...
		 		cloudlet.addOnFinishListener(info -> {
		 			...
		 			CM.InitialvmListDaas.remove(vm);
